...
local function toggle_scratchpad()
  ...
  for _, client in pairs(client.get()) do
    -- this following line changes it to use urxvt. Would need to comment out the next line to stop it form using kitty. 
    -- would also need to go down and uncomment the scratchpad rules for urxvt scratchpad in the window rules area.
    --if client.class == "URxvt" and client.instance == "scratchpad" then
    if client.class == "scratchpad" and client.instance == "scratchpad" then
  ...
  if scratchpad_terminal then
    ...
    -- Uncomment this line and comment out next line to use urxvt instead of kitty
    --awful.spawn("urxvt -name scratchpad -geometry 120x35", false)
    awful.spawn("kitty --class scratchpad", false)
  end
end
...
globalkeys = gears.table.join(
    ...
    -- Volume Control
    awful.key({ }, "XF86AudioRaiseVolume", function ()
              awful.util.spawn("wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%+") end),
    awful.key({ }, "XF86AudioLowerVolume", function ()
              awful.util.spawn("wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%-") end),
    awful.key({ }, "XF86AudioMute", function ()
              awful.util.spawn("wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle") end),
    ...
    -- Prompt
    awful.key({ modkey },            "d",     function () awful.spawn("rofi -show drun -font 'mono 14' -theme-str 'window{border-color: " 
...
awful.rules.rules = {
    ...
    -- I don't use this anymore, since kitty works...
    -- Scratchpad rule
    ...
    -- Kitty scratchpad rules
     { rule = { class = "scratchpad", instance = "scratchpad" },
      properties = {
        ontop = true,
        sticky = true,
        skip_taskbar = true,
      ...
      callback = function(c)
        c:geometry({ width = 1300, height = 800 })
        awful.placement.centered(c, { honor_workarea=true, honor_padding=true })
      end,
    },
